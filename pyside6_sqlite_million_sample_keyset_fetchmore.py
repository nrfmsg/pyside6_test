...
class SqliteTableModel(QAbstractTableModel):
    ...
    この版は次の2点を組み合わせている。
    1) キーセット方式で取得（WHERE id >= ? ORDER BY id LIMIT ?）
    2) canFetchMore/fetchMore で段階的に行を公開
    """
    ...
    def _ensure_cache(self, row: int) -> None:
        ...
        # OFFSET を使わず、id を起点に必要範囲のみ取得する（キーセット方式）。
        # 前提: このサンプルは INSERT のみで id が連番増加する。
