...
BATCH_SIZE = 10_000
# View へ段階的に公開する行数（fetchMore 1回分）
...
class SqliteTableModel(QAbstractTableModel):
    ...
    この版は canFetchMore/fetchMore を実装し、行を段階的に公開する。
    （DB全件があっても、View には必要分を順次見せる）
    """
    ...
    def __init__(self, db_path: Path, parent=None) -> None:
        ...
        self._total_row_count = self.conn.execute("SELECT COUNT(*) FROM texts").fetchone()[0]
        # View へ公開済みの件数。fetchMore により増える。
        ...
        # 初期表示に必要な最小件数だけ先に公開する。
        if self._total_row_count > 0:
            self._loaded_row_count = min(FETCH_BATCH_SIZE, self._total_row_count)
    ...
    def data(self, index: QModelIndex, role: int = Qt.DisplayRole):
        ...
        if not index.isValid() or role != Qt.DisplayRole:
            return None
        # まだ公開していない行へアクセスされた場合は空を返す。
        if index.row() >= self._loaded_row_count:
            return None
    ...
    def canFetchMore(self, parent: QModelIndex = QModelIndex()) -> bool:
        ...
        if parent.isValid():
            return False
        return self._loaded_row_count < self._total_row_count
    ...
    def fetchMore(self, parent: QModelIndex = QModelIndex()) -> None:
        ...
        remaining = self._total_row_count - self._loaded_row_count
        items_to_fetch = min(FETCH_BATCH_SIZE, remaining)
        if items_to_fetch <= 0:
            return

        first = self._loaded_row_count
        last = self._loaded_row_count + items_to_fetch - 1
        # begin/endInsertRows で「行が増えた」ことを View へ通知する。
        self.beginInsertRows(QModelIndex(), first, last)
        self._loaded_row_count += items_to_fetch
        self.endInsertRows()
    ...
    def _ensure_cache(self, row: int) -> None:
        ...
        # LIMIT/OFFSET で必要範囲のみを取得。
        # ORDER BY id を固定し、行番号と表示内容の対応を安定させる。
        rows = self.conn.execute(
            "SELECT id, value FROM texts ORDER BY id LIMIT ? OFFSET ?",
            (self._cache_size, start),
        ).fetchall()
        # 新しいキャッシュへ置き換える。
